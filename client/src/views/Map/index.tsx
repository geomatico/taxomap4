...
import {INITIAL_TAXONOMIC_ID, INITIAL_TAXONOMIC_LEVEL} from '../../config';
...
const Index: FC<IndexProps> = ({isTactile}) => {
  ...
  useEffect(() => {
    if (!level || !id) {
      ...
      if (taxonDictionaries[INITIAL_TAXONOMIC_LEVEL].length) {
        navigate(`${INITIAL_TAXONOMIC_LEVEL}/${INITIAL_TAXONOMIC_ID}`);
      }
  ...
  const childrenItems = useMemo(
    () => getTaxonChildren(subtaxonCount, selectedTaxon, taxonDictionaries),
    [subtaxonCount, selectedTaxon, taxonDictionaries]);
